...
export function EditCampaignPage() {
  ...
  // Campaign type (read-only after creation)
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([camp, catalog]) => {
        ...
        setIsFreeShipping(camp.isFreeShipping);
        setCodeUsageLimit(camp.codeUsageLimit ? String(camp.codeUsageLimit) : '');
        setNumberOfCodesToGenerate(1);
        setSelectedProducts(camp.productIds);
        setSelectedDefaultAssets(camp.defaultAssetItems);
        setSelectedBundles(camp.unlockableMenuItems);
        setColor1(camp.color1 || '#435268');
        setColor2(camp.color2 || '#a78bfa');
        setColor3(camp.color3 || '#34d399');
        setExistingLogoURL(camp.logoURL);
      })
      .catch((err) => {
  ...
  async function handleSubmit() {
    ...
    const payload: Partial<CreateCampaignRequest> = {
      name: name.trim(),
    ...
    try {
      await campaignService.update(campaignId!, payload, logoFile ?? undefined);
      addToast('success', 'Campaign updated successfully!');
      navigate(`/campaigns/${campaignId}`, { replace: true });
    } catch (err: unknown) {
      const e = err as { data?: { message?: string } };
      addToast('error', e?.data?.message || 'Failed to update campaign');
    } finally {
      setSubmitting(false);
    }
  }

  const typeLabel = campaignType === 'ProductFlowSingleUser' ? 'Single User'
    : campaignType === 'ProductFlowMultiUser' ? 'Multi User' : 'Asset Only';
  ...
  return (
    <div className="page-container">
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <Button variant="ghost" size="sm" onClick={() => navigate(`/campaigns/${campaignId}`)}>← Back</Button>
          <h2 className="section-title" style={{ marginTop: 8 }}>Edit Campaign</h2>
          <p className="section-subtitle">Type: <strong>{typeLabel}</strong> (cannot be changed)</p>
        </div>
      </header>
      ...
      <form onSubmit={(e) => e.preventDefault()}>
        <div style={{ display: 'grid', gap: 20 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                {logoPreview || existingLogoURL ? (
                  <img src={logoPreview || existingLogoURL!} alt="Logo preview" style={{ width: 80, height: 80, objectFit: 'contain', borderRadius: 8, border: '1px solid var(--color-border)' }} />
                ) : (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  {(logoFile || existingLogoURL) && <button type="button" onClick={() => { setLogoFile(null); setLogoPreview(null); setExistingLogoURL(null); }} style={{ background: 'none', border: 'none', color: 'var(--color-error)', cursor: 'pointer', fontSize: '0.85rem', textAlign: 'left' }}>Remove</button>}
                  {logoError && <div className="text-error" style={{ fontSize: '0.8rem' }}>{logoError}</div>}
                  <div className="text-secondary" style={{ fontSize: '0.8rem' }}>PNG or JPG, max 5MB. For best results, upload a 512×512px image. Leave empty to keep current logo.</div>
                </div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '8px 0 24px' }}>
            <Button variant="ghost" onClick={() => navigate(`/campaigns/${campaignId}`)}>Cancel</Button>
